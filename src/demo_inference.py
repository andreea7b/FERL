...
class DemoInference(object):
	...
	def __init__(self, loadfile):
		...
		feat_list = params["setup"]["feat_list"]
        weights = np.zeros((1, len(feat_list))
		self.environment = Environment(model_filename, object_centers, feat_list, weights)
		...
		self.learner = DemoLearner(self.environment, constants)
		...
		if self.demo_spec == "simulate":
			...
			self.environment.weights = params["sim"]["task"]["feat_weights"]
