...
class DemoLearner(object):
	...
	def __init__(self, environment, constants):
		...
		self.feat_range = [FEAT_RANGE[self.environment.feature_list[feat]] for feat in range(self.environment.num_features)]
		...
		self.weights_list = list(itertools.product(weight_vals, repeat=self.environment.num_features))
		if (0.0,)*len(self.environment.feature_list) in self.weights_list:
			self.weights_list.remove((0.0,) * self.environment.num_features)
		...
		for rand_i, traj_str in enumerate(self.traj_rand.keys()):
			...
			rand_features = self.environment.featurize(curr_traj)
	...
	def learn_weights(self, trajs):
		...
		new_features = [np.sum(self.environment.featurize(traj.waypts), axis=1) for traj in trajs]
