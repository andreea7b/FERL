		...
		if interaction:
			if self.reached_start and not self.reached_goal:
				...
				if self.feat_method == "beta":
					betas = self.learner.learn_betas(self.traj, torque_curr, timestamp)
				else:
					betas = [1.0] * self.environment.num_features
				self.learner.learn_weights(self.traj, torque_curr, timestamp, betas)
